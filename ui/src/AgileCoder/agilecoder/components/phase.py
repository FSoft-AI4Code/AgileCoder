...
class Phase(ABC):
    ...
    def chatting(
            self,
            chat_env,
            task_prompt: str, #use this
            assistant_role_name: str,
            user_role_name: str,  #use this
            phase_prompt: str,  #use this
            phase_name: str,
            assistant_role_prompt: str, # use this
            user_role_prompt: str, #use this
...
class ProductBacklogCreating(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            ...
            chat_env.env_dict['product-backlog'] = product_backlog
            chat_env.env_dict['acceptance-criteria'] = acceptance_criteria
        print("chat_env.env_dict['product-backlog']", chat_env.env_dict['product-backlog'],type(chat_env.env_dict['product-backlog']))
        write_context(chat_env.env_dict['product-backlog'],self.name)
        return chat_env
...
class SprintBacklogCreating(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        print("chat_env.env_dict['current-sprint-backlog']", chat_env.env_dict['current-sprint-backlog'],type(chat_env.env_dict['current-sprint-backlog']))
        write_context(chat_env.env_dict['current-sprint-backlog'],self.name)
        print("chat_env.env_dict['current-sprint-goals']", chat_env.env_dict['current-sprint-goals'],type(chat_env.env_dict['current-sprint-goals']))
        write_context(chat_env.env_dict['current-sprint-goals'],self.name)
        return chat_env
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        for task in list_of_sprint_backlog_items:
            ...
            if task.strip().startswith('-'):
                ...
                task_id += 1
        chat_env.env_dict['current-programming-task'] = '\n'.join(current_tasks)
        print("chat_env.env_dict['current-sprint-backlog']", chat_env.env_dict['current-sprint-backlog'],type(chat_env.env_dict['current-sprint-goals']))
        write_context(chat_env.env_dict['current-sprint-backlog'],self.name)
        print("chat_env.env_dict['current-sprint-goals']", chat_env.env_dict['current-sprint-goals'],type(chat_env.env_dict['current-sprint-goals']))
        write_context(chat_env.env_dict['current-sprint-goals'],self.name)
        return chat_env
...
class RolesEngagement(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        write_context(chat_env.env_dict['current-assigned-tasks'],self.name)
        return chat_env
...
class SprintReview(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        ...
        print('done work:', done_work,type(done_work))
        write_context(done_work,self.name)
        print('undone work:', type(undone_work))
        write_context(undone_work,self.name)
        return chat_env
...
class ProductBacklogModification(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        print('ProductBacklogModification:', self.seminar_conclusion,type(self.seminar_conclusion))
        write_context(self.seminar_conclusion,self.name)
        if len(self.seminar_conclusion) > 0:
            lists_of_backlog_items = self.seminar_conclusion.splitlines()
            lst = []
            flag = False
